...
| `coverage_history.py` (**ADR-008**, table half) | **Real, against a real workspace** | The point-in-time report is now also durable: `workspace.platform.coverage_history`, one row per dataset per run, appended (never `UPDATE`d/`DELETE`d) via `ucmeta coverage --publish-history`, off by default. Bootstrapped by `scripts/bootstrap_coverage_history.sql` and confirmed live with two real runs landing six rows, both present, the outcome measure's `simulated` label carried into every row. The native AI/BI dashboard over this table is the still-open half of **ADR-008**, not yet built. |
