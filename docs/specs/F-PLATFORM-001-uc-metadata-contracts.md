...
- *Security*: no long-lived personal access tokens. Locally, credentials come from an OAuth
  ...
  real business data.
- *Regulatory*: no validated or GxP regime is assumed, and none is designed for. The prototype
...
| Do we assume a validated / GxP regulatory regime and build a change-control record format, or record the raw audit material and flag the format as an open question? | hector | 2026-09-17 | *resolved: assume no GxP or validated regime at all.* Stronger than merely deferring the format question: no regulatory regime is assumed, designed for, or anticipated, and the previous "if the target org runs GxP" hedge is removed from Non-functional Requirements. The audit rigor stays — model, prompt, inputs, timestamp, approver, plus the immutable release record — but justified as ordinary engineering hygiene for a platform writing to a single global production metastore. Designing to a regime nobody has stated applies would be fabricated rigor. |
